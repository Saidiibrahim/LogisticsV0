...
import {
  ...
  selectIsSidebarOpen,
  selectIsSidebarOpenMobile,
  selectSetSidebarIsMobile,
  selectSetSidebarOpen,
  selectSetSidebarOpenMobile,
  selectSidebarState,
  selectToggleSidebar,
  useSidebarStore,
...
function SidebarProvider({
  ...
  const setIsMobileInStore = useSidebarStore(selectSetSidebarIsMobile);
  const setOpenInStore = useSidebarStore(selectSetSidebarOpen);
  const setOpenMobileInStore = useSidebarStore(selectSetSidebarOpenMobile);
  const open = useSidebarStore(selectIsSidebarOpen);
  const [hasHydrated, setHasHydrated] = React.useState(
    () => useSidebarStore.persist?.hasHydrated?.() ?? false
  );

  useSidebarKeyboardShortcut();

  React.useEffect(() => {
    const unsubscribe = useSidebarStore.persist?.onFinishHydration?.(() => {
      setHasHydrated(true);
    });
  ...
  React.useEffect(() => {
    setIsMobileInStore(isMobile);
    if (!isMobile) {
      setOpenMobileInStore(false);
    }
  }, [isMobile, setIsMobileInStore, setOpenMobileInStore]);

  React.useEffect(() => {
    if (typeof openProp === "boolean") {
      setOpenInStore(openProp);
      return;
    }

    if (!hasHydrated) {
      setOpenInStore(defaultOpen);
    }
  }, [defaultOpen, hasHydrated, openProp, setOpenInStore]);
...
function Sidebar({
  ...
  const openMobile = useSidebarStore(selectIsSidebarOpenMobile);
  const setOpenMobile = useSidebarStore(selectSetSidebarOpenMobile);

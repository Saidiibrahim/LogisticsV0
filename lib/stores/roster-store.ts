...
import { loadRosterForWeek, saveRoster } from "@/app/(authenticated)/calendar/roster-actions"
...
interface RosterState {
  ...
  saveCurrentRoster: (status: RosterStatus) => Promise<void>
  ...
  devtools(
    (set, get) => ({
      ...
      async loadRosterForWeek(weekStartDate) {
        const weekStartISO = toDateKey(startOfWeek(weekStartDate, { weekStartsOn: 1 }))
      ...
      updateAssignment(dateStr, driverId) {
        const { currentRoster } = get()
        if (!currentRoster) return
        const others = currentRoster.assignments.filter((a) => a.date !== dateStr)
        const next: RosterAssignment[] = [...others, { date: dateStr, driverId }]
      ...
      bulkUpdateAssignments(map) {
        ...
        const added = Object.entries(map).map(([date, driverId]) => {
          const existing = currentRoster.assignments.find((a) => a.date === date)
          return {
            date,
            driverId,
      ...
      getAffectedDriverIds() {
        ...
        return Array.from(new Set(changes.map((c) => c.newDriverId).filter(Boolean))) as string[]
      },

      isDateNotified(dateStr, driverId) {
        const { currentRoster } = get()
        const a = currentRoster?.assignments.find((x) => x.date === dateStr && x.driverId === driverId)
        return !!a?.notifiedAt
      },
